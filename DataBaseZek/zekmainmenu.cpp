...
{
    try{
    QFile *f = new QFile("D:/Qt bazaZek/ZekList.txt");
...
void ZekMainMenu::slotShowZekInfo(int i)
{
    ...
    if(colZek)
    {
        ...
        QFile *f = new QFile("D:/Qt bazaZek/"+ui->listWidget->currentItem()->text()+".txt");
        if(!f->open(QFile::ReadOnly|QFile::Text))
            throw new ErrorOpenFile;
        QTextStream *stream = new QTextStream(f);
        ...
        f->close();
        QPixmap *p = new QPixmap("D:/Qt bazaZek/"+ui->listWidget->currentItem()->text()+".jpg");
...
void ZekMainMenu::slotDeleteZek()
{
    ...
    QFile::remove("D:/Qt bazaZek/"+ui->listWidget->currentItem()->text()+".txt");
    QFile::remove("D:/Qt bazaZek/"+ui->listWidget->currentItem()->text()+".jpg");
...
void ZekMainMenu::slotStatia()
{
    ...
    QFile *f = new QFile("D:/Qt bazaZek/Statii/"+ui->textEdit_22->toPlainText()+".txt");
    if(f->open(QFile::ReadOnly|QFile::Text)){
    QTextStream *stream = new QTextStream(f);
